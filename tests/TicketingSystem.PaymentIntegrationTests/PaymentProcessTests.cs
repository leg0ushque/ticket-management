...
using TicketingSystem.IntegrationTests;
using TicketingSystem.WebApi.Controllers;
using TicketingSystem.WebApi.Models;
using Xunit;

namespace TicketingSystem.PaymentIntegrationTests
{
    ...
    public class PaymentProcessTests(DatabaseFixture fixture) : FixtureTestsBase(fixture), IClassFixture<DatabaseFixture>
    {
        ...
        public async Task GivenPaymentProcess_WhenPaymentSucceeded_ShouldUpdateSeatStateToSoldAndUpdatePaymentAndDeleteCartItems(bool paymentShouldBeSuccessful)
        {
            ...
            var cartItemsResult = await OrdersController.GetCartItems(paymentId) as OkObjectResult;
            ...
            cartItems.FirstOrDefault(i => i.EventSeatId == firstSeat.Id)
                .Should().BeEquivalentTo(
                    new CartItemDto
                    {
                        EventRowNumber = firstSeat.RowNumber,
                        EventSeatId = firstSeat.Id,
                        EventSeatNumber = firstSeat.SeatNumber,
                        Price = firstSeat.Price
                    });

            cartItems.FirstOrDefault(i => i.EventSeatId == secondSeat.Id)
                .Should().BeEquivalentTo(
                    new CartItemDto
                    {
                        EventRowNumber = secondSeat.RowNumber,
                        EventSeatId = secondSeat.Id,
                        EventSeatNumber = secondSeat.SeatNumber,
                        Price = secondSeat.Price
                    });
